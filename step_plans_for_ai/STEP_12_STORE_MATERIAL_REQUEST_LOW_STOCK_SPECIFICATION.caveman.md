# STEP_12_STORE_MATERIAL_REQUEST_LOW_STOCK_SPECIFICATION.caveman.md
...
**Target Module:** `solar_module` / `manoj` (Extends ERPNext `tabMaterial Request`, `tabMaterial Request Item`, `tabItem`, `tabItem Reorder`, `tabBin`; introduces `tabSolar Low Stock Incident Log`, `tabSolar Reorder Policy`)  
...
Step 12 primary demand engine for Solar EPC Procurement Lifecycle (Flow 2). Bridges engineering design demand from Step 03 ([`STEP_03_SURVEY_ENGINEERING_DESIGN_BOM_SPECIFICATION.md`](./STEP_03_SURVEY_ENGINEERING_DESIGN_BOM_SPECIFICATION.md)) + warehouse stock health with sourcing in Step 13 ([`STEP_13_SUPPLIER_RFQ_SPECIFICATION.md`](./STEP_13_SUPPLIER_RFQ_SPECIFICATION.md)), Step 14 ([`STEP_14_QUOTATION_COMPARISON_MATRIX_SPECIFICATION.md`](./STEP_14_QUOTATION_COMPARISON_MATRIX_SPECIFICATION.md)), and Step 15 ([`STEP_15_PURCHASE_ORDER_AUTHORIZATION_SPECIFICATION.md`](./STEP_15_PURCHASE_ORDER_AUTHORIZATION_SPECIFICATION.md)).
...
- **Predecessors:** Step 03 Survey Design BOM freeze (`STEP_03`), Step 06 Sales Order baseline (`STEP_06`), Stock Entries / Delivery Notes.
- **Successors:** Step 13 Supplier RFQ (`STEP_13`), Step 14 Supplier Quotation Comparative Matrix (`STEP_14`), Step 15 PO Placement (`STEP_15`).

### 1.2 Core Business Objectives & Target KPIs

1. **0% Project Stalls Due to Stockouts:** Critical solar install items never hit zero balance during active site execution.
2. **Sub-24-Hour Requisition Turnaround:** Average time from MR submission to RFQ release $< 24\text{h}$ ($< 4\text{h}$ for emergency indents).
3. **100% Elimination of Unbudgeted Emergency Freight:** Cut air-freight premiums and retail surcharges via lead-time compliant replenishment.
4. **Zero Duplicate Procurement:** Prevent overlapping purchase requisitions across identical SKUs and regional warehouses.
5. **Mathematically Grounded Replenishment:** Replace subjective guesses with true pipeline solvency math factoring open POs, open MRs, project reservations, and supplier lead times.

### 1.3 Context Traceability Matrix

| Requirement Ref | Source Document | Requirement Summary                                | Implementation in Step 12                                                                                                    |
| :-------------- | :-------------- | :------------------------------------------------- | :--------------------------------------------------------------------------------------------------------------------------- |
| **`BR-012`**    | `05_BRD.md`     | Store Requisition & Low-Stock Alerts               | Extended `tabMaterial Request` with project links, urgency tiers, automated low-stock incident logging.                      |
...
| **`BR-018`**    | `05_BRD.md`     | Multi-Tier Notification Engine                     | Automated multi-channel alerts (WhatsApp, Email, Desk) dispatched via `StoreNotificationBroker`.                             |
| **`FR-012`**    | `06_FRS.md`     | Store Requisition & Automated Low-Stock Monitoring | Screen controls, dual-trigger ingestion, project BOM headroom netting, automated low-stock workbench.                        |
| **`FR-017`**    | `06_FRS.md`     | SLA Engine & Overdue Tracking                      | Automated Redis cron monitoring SLA deadlines, marking breaches as `Overdue`, enforcing justification.                       |
| **`FR-018`**    | `06_FRS.md`     | Notification Matrix & Alert Toggles                | Dynamic recipient routing based on item urgency and Class-A classifications, governed by `tabSolar Notification Settings`.   |
...
| **`MOD-12`**    | `11_MOD.md`     | Module Functional SOP for Stores & SCM             | Step-by-step SOP for warehouse storekeepers, purchase assistants, inventory managers.                                        |

### 1.4 Failure Modes Eliminated

- **Informal "WhatsApp Requisitions":** Unrecorded, ad-hoc requisitions via phone/chat leading to unverified purchasing, untracked budgets, lost accountability.
- **Naive Stock Count Errors:** Re-ordering materials already in transit via open POs, or failing to re-order because on-hand stock counted without deducting active project reservations.
- **Class-A Asset Freight Chaos:** Requisitioning solar modules and string inverters without standard pallet packaging unit rounding, causing transit breakage and freight penalties.
- **Unfeasible Emergency Requisitions:** Requesting custom mounting structures with 2-day delivery when supplier lead time is 14 days.
- **Requisition Silos & Queue Stagnation:** Material Requests sitting indefinitely in unassigned procurement inboxes without automated escalation.
...
| :-------------------------------- | :------------------- | :--------------------- | :---------------------------- | :------------------------------------------------------------------------------------------------------------------------------------- |
| **Warehouse Storekeeper**         | `Store Assistant`    | Store & Logistics      | `Store Assistant`             | Performs bin stock counts, drafts manual project-linked Material Requests, verifies physically received stock, monitors safety levels. |
| **Inventory & Logistics Head**    | `Store Manager`      | Store & Logistics      | `Warehouse Manager`           | Reviews and approves drafted Material Requests, authorizes warehouse transfers, manages reorder policies, monitors incident logs.      |
| **Procurement Line Executive**    | `Purchase Assistant` | Purchase & SCM         | `Purchase Executive`          | Ingests approved Material Requests, drafts Supplier RFQs (Step 13), collects quotes, prepares comparative sheets (Step 14).            |
...
| **Site Technical Requisitioner**  | `Project Engineer`   | Engineering Operations | `Site Supervisor`             | Initiates site-specific material demands against approved Survey Engineering Design BOMs during active installation.                   |
| **Solar EPC Director / Admin**    | `Admin`              | Executive Management   | `Managing Director`           | Project supreme operational command; manages `Solar SLA Settings`, `Solar Notification Settings`, approves SLA overrides.              |
...
_\*Note: Frappe `Administrator` and `System Manager` apex hierarchy; inherit all operational permissions + technical code/schema access._
...
| :------------------------- | :---------------------------- | :----------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :-------: | :----------: | :--------------------------------------------------------------------------------------------- |
| `custom_project_reference` | Solar Project Reference       | `Link`       | `Project`                                                                                                                                                                    |    No     | **Index: 1** | Foreign key linking Solar EPC installation project.                                            |
| `custom_sales_order`       | Sales Order Reference         | `Link`       | `Sales Order`                                                                                                                                                                |    No     | **Index: 1** | Commercial baseline anchor for project procurement.                                            |
...
| `item_group`                 | Item Group                | `Link`     | `Item Group`                                                |  **Yes**  | **Index: 1** | Category (Solar Modules, Inverters, DC Cables, Mounting Structure). |
| `warehouse`                  | Warehouse                 | `Link`     | `Warehouse`                                                 |  **Yes**  | **Index: 1** | Warehouse location.                                                 |
| `incident_datetime`          | Breach Timestamp          | `Datetime` | -                                                           |  **Yes**  | **Index: 1** | System timestamp of low-stock detection.                            |
...
| Fieldname                 | Label                      | Fieldtype  | Options / Target | Mandatory |    Index     | Description                                      |
| :------------------------ | :------------------------- | :--------- | :--------------- | :-------: | :----------: | :----------------------------------------------- |
| `policy_name`             | Policy Name                | `Data`     | -                |  **Yes**  | **Index: 1** | Descriptive title.                               |
...
- **Condition:** For every item in `doc.items`, query `tabMaterial Request Item` joined with `tabMaterial Request` where:
  ...
  - `name != doc.name`
- **Rule:** If active unfulfilled requisition exists, submission hard-blocked:
  $$\text{Raise } \texttt{DuplicateRequisitionError}$$
- **Override:** Allowed only if `custom_urgency_level = 'Critical Breakdown'` with explanation in `custom_delay_reason`.
...
- **Rule:** Requisitions requesting delivery faster than supplier lead time rejected.
- **Override:** Expedited delivery dates accepted only with `Purchase Manager` or `Admin` sign-off, tagged `Urgent` or `Critical Breakdown`.

#### Gate 3: Project BOM Headroom Netting Gate

- **Execution Hook:** `MaterialRequest.validate()`
- **Condition:** When `custom_project_reference` set:
  1. Retrieve approved Survey Engineering Design BOM (`STEP_03`) for project.
  2. Compute total authorized BOM quantity for SKU ($\text{BOM\_Qty}$).
  3. Query total quantity issued via Delivery Notes ($\text{Issued\_Qty}$) + quantity on open submitted MRs ($\text{Open\_MR\_Qty}$).
  4. Assert:
     $$\text{row.qty} \le \text{BOM\_Qty} - \text{Issued\_Qty} - \text{Open\_MR\_Qty}$$
- **Rule:** Blocks requisitioning materials exceeding design BOM allocations without approved ECO.

#### Gate 4: Closed-Loop Downstream Procurement Traceability Gate

- **Execution Hook:** Step 13, 14, 15 linkages.
- **Rule:** Downstream `Request for Quotation` (Step 13) and `Purchase Order` (Step 15) must carry foreign key link `custom_material_request_ref`. Step 14 comparative evaluation sheet links quotation candidates before PO award.

### 4.3 Tiered Turnaround SLA & Escalation Engine

SLA countdown durations customized by `Admin` in `tabSolar SLA Settings` and enforced via scheduled Redis background workers running every 15 minutes (`solar_module.tasks.recompute_enterprise_slas`):

| Requisition Urgency Level        | Target SLA Duration | Primary Assigned Role | Escalation Role (T-Breach)   | Breach Consequence                                                 |
| :------------------------------- | :-----------------: | :-------------------- | :--------------------------- | :----------------------------------------------------------------- |
| **Critical Breakdown**           |     **4 Hours**     | `Purchase Assistant`  | `Purchase Manager` & `Admin` | Urgent WhatsApp to Director; sets `custom_sla_status = 'Overdue'`. |
...
- **Mandatory Delay Audit Protocol:** When `custom_sla_status = 'Overdue'`, document edits or status transitions hard-blocked until valid explanation appended to `tabSolar Stage Delay Log` (`tabRemark-Delay Log`).
...
def monitor_low_stock_scheduled_task():
	...
	Scans warehouse bins configured with reorder rules,
	evaluates true pipeline solvency, logs incidents, auto-generates draft MRs.
	"""
...
def create_material_request(payload: dict) -> dict:
	"""
	Whitelisted API to create and optionally submit Store Material Request.
...
def get_low_stock_workbench_data(warehouse: str = None, item_group: str = None) -> list:
	"""
	Fast batch-query endpoint powering Vue 3 / Frappe UI Low-Stock Workbench.
...
Operates under Unified Frontend Landing Architecture ([`planning_ref_docs/README.prd.md`](../planning_ref_docs/README.prd.md#unified-frontend-landing-architecture--desk-access-governance)). Store and SCM users land on `/solar` Vue 3 + Frappe UI SPA. Direct navigation to `/desk` or `/app` redirected to `/solar`; deep links role-gated.
...
- **Header KPI Tiles:**
  ...
  - `Class-A Solar Assets Indented` (PV modules/inverters pending PO release).
- **Interactive Workbench Filter:**
  - Filter: Target Warehouse, Requisition Urgency, Trigger Source, Project Link.
  - Action: "+ Raise Project Indent" (modal opening BOM netting selector).
- **Data Table:**
  ...
  - Action: "View Downstream SCM Pipeline" drawer (linked RFQ, Quote Comparison, PO, GRN).
...
- **Batch Action Toolbar:**
  ...
  - "Batch Raise Requisitions" button with modal displaying replenishment quantities rounded to pallet packing units.

### 6.2 Frappe Desk View Customizations (`material_request.js`)

When authorized users open desk deep link (`/app/material-request/:id`):

1. **Dynamic Custom Buttons:**
   - `Check Multi-Warehouse Stock`: Modal showing live `Bin` quantities (`actual_qty`, `ordered_qty`, `reserved_qty`, `projected_stock`) across stores.
   - `View Linked Downstream Documents`: Pops up linked Step 13 RFQs, Step 14 Comparison Matrices, Step 15 POs.
   - `Log Incident History`: Opens timeline showing threshold breach logs for items.
2. **Visual SLA Badge:** Renders color-coded countdown widget at top (`Within SLA`, `Grace Period`, `Overdue`).
3. **Mandatory Delay Reason Field:** If `custom_sla_status == 'Overdue'`, dynamically enforces `custom_delay_reason` mandatory before submit.
...
Inherits from `frappe.tests.utils.FrappeTestCase` or `IntegrationTestCase`. Tests run within transactional isolation, rollback upon teardown. **Zero `frappe.db.commit()`.**
...
class TestStep12StoreMaterialRequest(FrappeTestCase):
	...
	def test_pallet_multiple_packaging_rounding(self):
		"""Asserts replenishment quantity rounds up to packaging unit multiples."""
...
1. **Daily Morning Stock Review:** Access `/solar/store/low-stock`. Review items flagged Amber and Red.
2. **Project Indent Creation:** Access `/solar/store/material-requests`, select "+ Raise Project Indent", link Solar EPC `Project`, select SKUs, verify quantities within BOM headroom.
3. **Lead Time Check:** Ensure `schedule_date` beyond supplier lead time. If emergency, set `custom_urgency_level = 'Critical Breakdown'` and alert Store Manager.
4. **Draft Submission:** Save `Draft` and notify Store Manager for sign-off.

#### For `Store Manager` (Warehouse Head):

1. **Requisition Review:** Review pending store requisitions. Validate warehouse allocation and inter-warehouse stock transfer viability before approving purchase indent.
2. **Document Sign-Off:** Submit document (`docstatus = 1`). Triggers notification to Purchase team and starts Turnaround SLA clock.
...
2. **Initiate RFQ (Step 13):** Group approved indents by category, click "Create RFQ" to release to $\ge 3$ approved suppliers.
3. **Supplier Quotation & Comparison (Step 14):** Collect vendor quotes, enter into `tabSupplier Quotation`, generate Comparative Matrix evaluating landed cost, delivery schedule, terms, and vendor scorecards.
