# STEP_02_SITE_SURVEY_SPECIFICATION.caveman.md
...
Stage 02 forms engineering baseline of Solar EPC Project Execution Lifecycle. Trigger on surveyor assignment in Stage 01 (`Lead Management`). Dispatches field engineer to customer site to capture roof structural feasibility, electrical infrastructure distances, shadow obstacles, GPS geotag coordinates, utility meter attributes, photo/video evidence in 24h SLA window.

Operates across remote agricultural terrains (PM KUSUM pumps), rural rooftops (PM Surya Ghar), industrial sheds with spotty connectivity. Implements **Offline-First PWA Mobile Architecture**. Form inputs, high-res photos, video walkthrough cached locally in IndexedDB until network returns, triggering resilient two-stage background auto-sync with fail-safe cache eviction.
...
1. **Zero Survey Revisits (100% First-Visit Completeness):** Enforce server-side validation preventing survey sign-off until 10 mandatory technical fields, 6 fixed photo checklist rows, utility meter alignment checks captured.
2. **Sub-24-Hour Completion Turnaround (SLA $\le 24\text{h}$):** Guarantee on-site data captured, synchronized, verified in $\le 24\text{h}$ of assignment.
3. **Zero Data Loss in Remote Environments (100% Offline Resilience):** Persistent client IndexedDB caching protects form fields, high-res photos, 360° video walkthroughs against app termination, device reboot, network dropouts.
4. **Geotagged Spatial Verification ($\le \pm 15\text{m}$ Accuracy):** Device GPS coordinates verified against registered site pincode and reverse-geocoded address.
5. **Upstream Electrical Sanction Compliance:** Sizing feasibility check ensuring requested solar PV capacity does not exceed DISCOM sanctioned load without load enhancement flag.

### 1.3 Failure Modes Eliminated

- **Field Data Loss from Network Dropouts:** Field auditors losing roof measurements and photos due to mobile connection resets in basement switchrooms or rural zones.
- **Repeat Site Inspections:** Design engineers rejecting survey submissions because surveyor missed LT panel busbar rating or meter board location.
- **DISCOM Liaisoning Rejections at Stage 10:** Discrepancies between electricity bill name and property documents discovered during net-meter sync.
- **Untracked Operational Bottlenecks:** Surveys lingering in "In Progress" indefinitely without automated SLA escalation or delay accountability.
...
| **Field Survey Assistant**     | `Survey Assistant`      | Engineering Operations    | `Field Operations Assistant` | Assists with physical roof tape measurements, shadow azimuth readings, cable path tracing.                        |
...
| **Solar EPC Director / Admin** | `Admin`, `Director`     | Executive Management      | `Managing Director`          | Supreme operational command; manages `Solar SLA Settings`, `Solar Notification Settings`, delay reason overrides. |
| **Technical DevOps Lead**      | `System Manager`        | Technology Infrastructure | `DevOps Architect`           | Framework apex; manages DocType schemas, Redis queues, background worker daemons, bench CLI tooling.              |
...
Standalone DocType (`tabSite Survey`). Technical single source of truth for downstream engineering:
...
| `assigned_by`               | Assigned By                     | `Link`         | `User`                                                                                                                                                                                                      |    No     |      -       | User who scheduled/assigned survey.                                   |
...
| `contact_mail`              | Customer Email Address          | `Data`         | `Email`                                                                                                                                                                                                     |    No     |      -       | Fetch `lead.email_id`.                                                |
...
| `site_type`                 | Roof / Civil Structure          | `Select`       | `RCC\nGround Mount\nShed (Profile Sheet)\nCar Port (Parking)\nRCC & Shed\nRCC & Ground Mount\nRCC & Parking\nShed & Ground Mount\nShed & Parking\nGround Mount & Parking\nRCC, Shed, Ground Mount, Parking` |  **Yes**  |      -       | Civil classification of installation surface.                         |
...
| `storage_space`             | Material Storage on Site        | `Select`       | `Yes\nNo`                                                                                                                                                                                                   |  **Yes**  |      -       | Confirms secure, covered space for staging at Stage 07.               |
...
| `panel_and_inverter_dst`    | Cable Distance: Array-Inverter  | `Float`        | -                                                                                                                                                                                                           |  **Yes**  |      -       | Length (m/ft) for DC cable sizing in Stage 03. Precision: 1.          |
| `panel_and_meter_dst`       | Cable Distance: Inverter-Meter  | `Float`        | -                                                                                                                                                                                                           |  **Yes**  |      -       | Length (m/ft) for AC cable sizing in Stage 03. Precision: 1.          |
...
1. **Gate 1: Geolocation & Coordinate Integrity Gate:**
   ...
   - Cross-checks site coordinates fall within $\le 50\text{km}$ radius of registered customer pincode.

2. **Gate 2: 10-Field Technical Audit Invariant Gate:**
   - Before `stage_status` transitions to `Completed`, validates:
...
3. **Gate 3: 6-Row Mandatory Photo & Video Checklist Verification Gate:**
   ...
   - Confirms none of 6 fixed rows deleted or altered.
   - Asserts every row with `is_mandatory = 1` has valid `upload_doc` URI referencing existing record in `tabFile`.
   - Asserts 360° video walkthrough attachment exists and file size is $\ge 1.0\text{MB}$.

4. **Gate 4: 24h SLA Breached Delay Reason Gate:**
   - If $\text{now\_datetime}() > \text{exp\_complete\_date}$, status transitions to `Overdue`.
   - When completing overdue survey, system asserts:
     $$\text{len}(\text{delay\_log}) \ge 20 \quad \text{OR} \quad \text{len}(\text{remark\_delay\_log}) > 0$$
   - Halts persistence with `frappe.ValidationError` if delay rationale missing.
...
- **Dynamic Calculation Rule:**
  - When `for_survey_assign_on` stamped, `exp_complete_date = for_survey_assign_on + 24 hours`.
- **Overdue Daemon (`solar_module.tasks.recompute_survey_sla`):**
  - Runs every 15 minutes across background workers:
    ```sql
  ...
  - Enqueues escalation alerts to assigned `Survey Engineer`, `Area Sales Manager`, `Admin`.
...
Extracted into dedicated Domain Services:
...
Mutating endpoints enforce `@frappe.whitelist(methods=["POST"])`, strict typing, in-method IDOR checks, rollback guarantees.
...
Touch-optimized SPA mounted inside `/solar` wrapper. Designed for mobile browsers in direct sunlight:
...
PWA integrates **Dexie.js** for persistent IndexedDB storage:
...
- **Lead Synchronization:** On completion, updates `tabLead.stage_status = 'Site Survey Completed'`, recording verified capacity and coordinates.
- **Customer & Address Staging:** Populates verified reverse-geocoded address, pin code, district into `Lead` so Stage 05 customer instantiation uses 100% clean data.

### 7.2 Frappe CRM Integration

- **Opportunity / Deal Pipeline:** Reflects Stage 02 technical clearance on CRM sales funnel.
- **Survey Summary Note:** Appends markdown note to CRM timeline with site capacity, mounting type, panorama link.

### 7.3 Frappe HRMS Integration

- **Employee Attribution:** `surveyed_by` links to `tabEmployee`.
- **Geofenced Check-In Assertion:** Cross-validates surveyor location against `tabEmployee Checkin` coordinates at shift start.

### 7.4 External Integrations & Statutory DISCOM Readiness

- **OpenStreetMap / Nominatim Reverse Geocoder:** Converts device lat/long into structured address.
- **Raven Message Notification Broker:** Broadcasts completion card to channel `Raven-survey-alerts` with thumbnail previews of 6 fixed photos.
- **WhatsApp Business API (WABA):** Sends automated notification acknowledging technical site survey completion and CAD design commencement.
- **PM Surya Ghar National Portal Readiness:** Captures electricity bill account number and meter board photo required for Phase 1 DISCOM filings.
...
All tests subclass `frappe.testing.IntegrationTestCase` (or `frappe.tests.utils.FrappeTestCase`). Atomic database transactions roll back automatically (`frappe.db.rollback()`). Zero database commits (`frappe.db.commit()`) permitted.
...
| :-------------------------------------------------------------------- | :--------------------------------------------------------------------- | :--------------------------------------------------------------------------------------------------------------------------- |
| **`ValidationError: Fixed row '{row}' cannot be deleted`**            | Auditor attempted to delete mandatory photo checklist row.             | Refresh document to restore all 6 standard photo rows. Never delete checklist items.                                         |
| **`ValidationError: Missing mandatory attachment for row '{row}'`**   | Attempted to complete survey with checklist item lacking photo upload. | Tap missing row, select camera icon, capture required photo, retry completion.                                               |
| **`ValidationError: GPS Accuracy too coarse (±{accuracy}m > 50m)`**   | Weak GPS lock, indoor location mode, cached mock location.             | Move outdoors away from tall concrete structures, wait 15 seconds for satellite triangulation, tap **Lock GPS** again.       |
| **`ValidationError: Overdue Survey requires mandatory Delay Reason`** | 24h turnaround window expired and survey completed past deadline.      | Go to **TAT & SLA Tracking**, enter detailed explanation in `delay_log` ($\ge 20$ chars), select delay reason, tap Complete. |
| **`SyncError: Network disconnected during media transmission`**       | Cellular signal dropped midway while uploading video or photo files.   | Do not close app. PWA automatically pauses and retains media in IndexedDB. Upload resumes automatically when 4G/5G returns.  |
...
- **Triage Steps:**
  1. Inspect Redis queue and Frappe background workers:
     ...
     ```
  2. Manually trigger survey SLA daemon:
     ```bash
...
- [x] **Section 1:** Step scope, remote field context, KPIs, failure modes clearly articulated.
- [x] **Section 2:** Stakeholders, HRMS designations, permission hierarchy mapped with zero ambiguity (Zero "User" Suffix Rule enforced).
- [x] **Section 3:** 3NF relational data dictionary complete with 10 mandatory fields, 6 fixed photo checklist rows, video row, composite indexes.
- [x] **Section 4:** State machine, 4 hard verification gates, 24h SLA engine, delay logging specified.
- [x] **Section 5:** Decoupled SOLID domain services, whitelisted POST APIs, chunked media ingestion, IDOR protection documented.
- [x] **Section 6:** Mobile touch PWA (`/solar/surveys/:id`), Dexie IndexedDB caching schema, two-stage auto-sync pipeline, fail-safe cache eviction protocol defined.
...
- [x] **Section 9:** Remote field surveyor SOP, operator error resolution matrix, L3 DevOps incident runbook ready for deployment.
