# STEP_06_SALES_ORDER_BASELINE_SPECIFICATION.caveman.md
...
**Target Module:** `solar_module` / `manoj` (Extend ERPNext `Sales Order`, `Sales Order Item`, `Project`, `Task`, + standalone `Liaisoning And Synchronization`)  
...
### 1.1 Lifecycle Positioning & Commercial-to-Execution Fulcrum

Stage 06 primary operational, legal, engineering anchor in Solar EPC lifecycle. Non-negotiable pivot: pre-sale commercial pipeline hand over project to execution teams, warehouse SCM, statutory liaisoning.

Trigger: Stage 05 financial advance clearance verified (`custom_advance_verified = 1`). Ingest finalized proposal (`Quotation`), bind verified `Customer`, freeze engineering BOM (`custom_quot_bom`), run atomic multi-department kickoff.
...
└──────────────────────────────────────────────────────────────────────────────────────────────────┘
  ┌───────────────────────┐
  │       STAGE 04:       │  Customer accept & finalize proposal
  │   PROPOSAL ENGINE     │  (custom_is_finalized = 1, required advance auto-calculated)
  │ (Quotation aliased)   │
  └───────────┬───────────┘
              │ [Generate advance terms: default ≥ 50%]
              ▼
  ┌───────────────────────┐
  │       STAGE 05:       │  Advance verified (Track A/B/C/D);
  │   ADVANCE CLEARANCE   │  Customer, Address & Contact incepted
  │   & CUSTOMER GATE     │  (custom_advance_verified = 1 / clearance date logged)
  └───────────┬───────────┘
              │ [Unlock Sales Order creation & submit]
              ▼
  ┌────────────────────────────────────────────────────────────────────────────────────────────────┐
  │                 STAGE 06: SALES ORDER MASTER BASELINE ANCHOR & SPAWNING                        │
  ├────────────────────────────────────────────────────────────────────────────────────────────────┤
  │ 1. Non-Bypassable Financial Gate: Assert custom_advance_verified == 1 from Stage 05           │
  │ 2. Signed Contract Integrity: Mandate attached signed agreement & formal contract date         │
  │ 3. Cryptographic Baseline Freeze: Compute SHA-256 hash across BOM & terms                      │
  │ 4. Milestone Payment Alignment: Generate 4-tier milestone schedule matching EPC milestones     │
  │ 5. Atomic Triple Downstream Spawning (Single ACID Transaction on Submission):                  │
  │    • Spawn ERPNext Project container with templated multi-zone WBS Tasks                       │
  │    • Spawn Material Delivery Task for Store Manager (reassignable to Store Assistant)          │
  │    • Spawn Liaisoning And Synchronization record (Phase 1 DISCOM Early Compliance)             │
  └───────────┬───────────────────────────────┬───────────────────────────────┬────────────────────┘
              │                               │                               │
              ▼                               ▼                               ▼
  ┌───────────────────────┐       ┌───────────────────────┐       ┌───────────────────────┐
  ...
  │  (Store Manager /     │       │   (Project Engineer / │       │  (Liaisoning Rep/Mgr  │
  │   Store Assistant)    │       │    Site Supervisor)   │       │    Phase 1 NOC Filing)│
  └───────────────────────┘       └───────────────────────┘       └───────────────────────┘
```

- **Predecessor:** Stage 05: Advance Payment Clearance & Customer Master Inception Gate ([`step_plans/STEP_05_ADVANCE_PAYMENT_CUSTOMER_GATE_SPECIFICATION.md`](./STEP_05_ADVANCE_PAYMENT_CUSTOMER_GATE_SPECIFICATION.md)), require `custom_advance_verified == 1` + verified `Customer`.
- **Successors:**
  - Stage 07: Material Dispatch via Delivery Note (`Delivery Note` require submitted, baseline-locked `Sales Order`).
  - Stage 08: Zone-Based Installation Execution & Mobile DPR (`Project` + WBS `Task` spawned).
  - Stage 10: Statutory Liaisoning & Synchronization (`Liaisoning And Synchronization` initialized in Phase 1).

### 1.2 Core Business Objectives & Target KPIs

1. **Zero Unbudgeted Scope Creep (100% Margin Protection):** Eliminate post-sale specs change, unapproved equipment swap, arbitrary discounts. Lock technical + commercial terms behind SHA-256 checksum.
2. **Sub-24h Kickoff TAT:** Cut kickoff delay (legacy 5-10 days) to seconds. Auto-provision project container, warehouse tasks, statutory compliance.
3. **100% Warehouse Material Visibility & Early Staging:** **`Store Manager`** receive formal **Material Delivery Task** immediately on order submit. Enable stock allocation, picking verification, delegation to **`Store Assistant`**.
4. **Early Statutory Feasibility (No Net-Metering Gridlock):** Trigger Phase 1 DISCOM filings (KYC, feasibility NOC, division portal registration) at Stage 06. Prevent commissioning hold-ups at Stage 10.
5. **Deterministic Cash Flow & Milestone Governance:** Auto-sync ERPNext `Payment Schedule` with EPC milestones (Advance $\ge 50\%$, Dispatch 20-30%, Installation 10-15%, Net-Metering 5-10%).
6. **Audit Non-Repudiation:** Mandate attached signed contract, timestamped baseline freeze, full actor attribution.

### 1.3 Operational Failure Modes Eliminated

| Failure Mode in Legacy / Standard ERP       | Root Cause                                                                                             | Stage 06 Engineered Resolution                                                                                                                   |
| :------------------------------------------ | :----------------------------------------------------------------------------------------------------- | :----------------------------------------------------------------------------------------------------------------------------------------------- |
| **Unbudgeted Scope Creep & Margin Erosion** | Verbal requests change panels, quantities, cable runs without cost re-estimate.                        | `SalesOrderBaselineService` compute SHA-256 hash over items/rates; lock baseline (`custom_baseline_frozen = 1`) on submit.                       |
| **Store & Logistics Blindspots**            | Storekeeper get no notice until site calls demanding urgent delivery; stock shortages.                 | Atomic spawning generate **Material Delivery Task** assigned direct to **`Store Manager`** with delegation to **`Store Assistant`**.             |
| **Delayed Net-Metering DISCOM Approvals**   | Statutory applications start only after physical installation; grid energization delayed 60-90 days.   | Sales order submit auto-create `Liaisoning And Synchronization` in Phase 1; dispatch task to `Liaisoning Representative` & `Liaisoning Manager`. |
| **Unfunded Project Procurement**            | Project managers create tasks and issue materials based on draft sales orders before advance verified. | Server-side gate assert `custom_advance_verified == 1`; block submit if Stage 05 gate incomplete.                                                |
| **Manual WBS Setup Inconsistencies**        | Project engineers manually create tasks with erratic naming, missing safety checks, wrong zones.       | `ProjectSpawnerService` programmatically create standard multi-zone WBS `Task` tree with dependencies and SLAs.                                  |
| **Mismatched Invoicing Milestones**         | Finance issue arbitrary billing schedules unrelated to contract milestones; customer disputes.         | Programmatic generation of ERPNext `Payment Schedule` linked to contract advance, dispatch, structural, grid milestones.                         |
...
| :-------------------------------- | :---------------------------------------- | :------------------------ | :------------------------------------------ | :------------------------------------------------------------------------------------------------------------------------- |
| **Sales Operations & Commercial** | `Sales Representative`                    | Sales & Marketing         | `Sales Representative`                      | Draft `Sales Order`, verify Stage 05 advance clearance, validate proposal alignment, attach signed contract, submit order. |
| **Sales Authority & Oversight**   | `Sales Manager`                           | Sales Management          | `Sales Manager`                             | Review commercial orders, authorize deviations, supervisory management and authority inheritance.                          |
| **Site Operations Lead**          | `Site Supervisor` / `Project Engineer`    | Project Engineering       | `Site Supervisor` / `Project Engineer`      | Receive spawned `Project` container, review zone segmentation, accept WBS execution tasks, schedule site mobilization.     |
| **Project Delivery Manager**      | `Project Manager`                         | Project Management        | `Project Manager`                           | Supervisory oversight of project execution, task schedules, and milestone delivery.                                        |
| **Warehouse & Inventory Head**    | `Store Manager`                           | Warehouse & Logistics     | `Store Manager` / `Warehouse Incharge`      | Receive spawned **Material Delivery Task**, evaluate stock against frozen BOM, reassign task to `Store Assistant`.         |
| **Warehouse Operations Staff**    | `Store Assistant`                         | Warehouse & Logistics     | `Store Assistant` / `Logistics Coordinator` | Receive delegated picking/packing tasks, prepare serialized dispatch bundles, coordinate transport.                        |
| **Statutory Compliance Lead**     | `Liaisoning Representative`               | Legal & Liaisoning        | `Liaisoning Representative`                 | Receive Phase 1 statutory dossier, gather DISCOM application docs, file online grid connectivity NOC.                      |
| **Statutory Compliance Head**     | `Liaisoning Manager`                      | Legal & Liaisoning        | `Liaisoning Manager`                        | Supervisory oversight of DISCOM interactions, CEIG clearances, and grid synchronization.                                   |
| **Finance Authority**             | `Accounts Assistant` / `Accounts Manager` | Accounts & Finance        | `Accounts Assistant` / `Accounts Manager`   | Reconcile advance payment entry, verify milestone payment terms, approve invoice schedules.                                |
| **Executive Supreme Command**     | `Admin`, `Director`                       | Executive Leadership      | `Managing Director` / `CEO`                 | Supreme operational command; authorize baseline amendments, configure `Solar Sales Order Settings`, override SLAs.         |
| **Technical DevOps Lead**         | `System Manager`                          | Technology Infrastructure | `DevOps Architect`                          | Framework apex; manage DocType schemas, custom fields, Property Setters, Redis queues, bench CLI.                          |

> [!IMPORTANT]
> **Enterprise Authority Hierarchy: Administrator $\rightarrow$ System Manager $\rightarrow$ Admin (Project Supreme):**
>
> - **`Administrator` & `System Manager` (Framework Supreme / Developer Realm):** Apex of system authority. Full access to code, schema builder, Server/Client Scripts, bench, Redis, dev mode.
> - **`Admin` (Project / Solar Supreme):** Operational supremacy across Flow 1 and Flow 2 docs. Exclusive authority over `Solar Sales Order Settings`, `Solar Advance Settings`, `Solar SLA Settings`. Authorize **Sales Order Baseline Amendments**. Restricted from code and schema changes.

### 2.2 Role Permission Matrix

| DocType / Action                         | Sales Representative | Sales Manager | Project Engineer / Site Supervisor | Store Manager  | Liaisoning Representative / Manager | Admin (Project Supreme) |
| :--------------------------------------- | :------------------: | :-----------: | :--------------------------------: | :------------: | :---------------------------------: | :---------------------: |
| **Sales Order (Read)**                   |     Full Access      |  Full Access  |            Full Access             |  Full Access   |              Read Own               |       All Records       |
| **Sales Order (Create/Edit)**            |    **Permitted**     | **Permitted** |             Restricted             |   Restricted   |             Restricted              |      **Permitted**      |
| **Sales Order (Submit / Lock Baseline)** |    **Permitted**     | **Permitted** |             Restricted             |   Restricted   |             Restricted              |       **Supreme**       |
| **Sales Order (Amend / Cancel)**         |      Restricted      |  Restricted   |             Restricted             |   Restricted   |             Restricted              |  **Yes (Admin Only)**   |
| **Project (Read)**                       |      Permitted       |   Permitted   |            Full Access             |   Read Only    |              Read Only              |       All Records       |
| **WBS Execution Tasks (Read/Update)**    |      Restricted      |  Restricted   |            **Assigned**            |   Restricted   |             Restricted              |       Full Access       |
| **Material Delivery Task (Read/Update)** |      Read Only       |   Read Only   |             Read Only              |  **Assigned**  |             Restricted              |       Full Access       |
| **Task Reassignment (Store Task)**       |      Restricted      |  Restricted   |             Restricted             | **Authorized** |             Restricted              |       **Supreme**       |
| **Liaisoning Dossier (Read/Update)**     |      Read Only       |   Read Only   |             Read Only              |   Restricted   |            **Assigned**             |       Full Access       |
| **SLA Delay Log Sign-Off**               |      Permitted       |   Permitted   |             Permitted              |   Permitted    |              Permitted              |       **Supreme**       |
...
Stage 06 extend ERPNext standard `tabSales Order` (`is_submittable = 1`) with solar attributes, baseline hash, downstream links:

| Fieldname                          | Label                     | Fieldtype    | Options / Target                                                                   | Mandatory | Unique / Index | Description & Validation Rules                                                  |
| :--------------------------------- | :------------------------ | :----------- | :--------------------------------------------------------------------------------- | :-------: | :------------: | :------------------------------------------------------------------------------ |
| `custom_is_solar_order`            | Is Solar EPC Order        | `Check`      | -                                                                                  |    Yes    |    Index: 1    | Flag identifying Solar EPC orders; toggle domain validations. Default: 1.       |
...
| `custom_baseline_frozen`           | Baseline Frozen           | `Check`      | -                                                                                  |    Yes    |    Index: 1    | Set to 1 on submit. Lock form from edits.                                       |
...
| `custom_zone_identifier`   | Installation Zone Name | `Data`     | -                                                                                                                                       |    No     |       -        | Physical roof/field partition (e.g. `Roof Zone 1`, `Ground Array A`).   |
...
Encapsulate statutory utility net-metering compliance across dual lifecycle timings (Domain 8: `CMP`):
...
```mermaid
stateDiagram-v2
    [*] --> Draft: Sales Representative / Sales Manager Create SO (from Finalized Proposal)
...
Before `Sales Order.on_submit()`, controller validates five verification gates:
...
- **Validation Rule:** Assert `custom_advance_verified == 1` and `custom_financial_clearance_date` not null.
- **Enforcement:** If `custom_advance_verified != 1`, raise blocking `frappe.ValidationError`:
...
- **Validation Rule:** `custom_signed_contract_doc` contain valid file attachment and `custom_contract_date` not null.
- **Enforcement:** Block verbal order approvals. Require customer signature on record.

#### Gate 3: Customer & Statutory Completeness Gate

- **Validation Rule:** Linked `Customer` master must have non-empty `custom_discom_consumer_no` and verified site billing/shipping address from Stage 05.

#### Gate 4: Dynamic Engineering BOM Integrity Gate

- **Validation Rule:** `SalesOrderBaselineService` compare items, quantities, equipment specs against frozen `custom_quot_bom` of linked `Survey Engineering Design`.
- **Enforcement:** Block unapproved substitution with `frappe.ValidationError`.

#### Gate 5: Milestone Payment Schedule Gate

- **Validation Rule:** Child table `tabPayment Schedule` lines must total $100\%$ of `grand_total`, Milestone 1 (Advance) matching collected Stage 05 advance ($\ge 50\%$).

### 4.3 Turnaround Time (TAT) SLA Engine & Delay Logging

- **Target SLA Clock:** **24 Hours** from timestamp of Stage 05 financial clearance (`custom_financial_clearance_date`).
- **Deadline Computation:**
  $$\text{custom\_so\_sla\_deadline} = \text{custom\_financial\_clearance\_date} + 24 \text{ hours}$$
- **State Escalation:**
  - If $\text{Current Datetime} \le \text{custom\_so\_sla\_deadline}$: Status `Within SLA` (Green badge).
  - If $\text{Current Datetime} > \text{custom\_so\_sla\_deadline}$ and unsubmitted: Update `custom_so_sla_status = 'Overdue'` (Red badge).
- **Mandatory Delay Reason Enforcement:** Submitting overdue `Sales Order` blocks until `custom_delay_reason_category` + `custom_delay_remarks` (min 20 chars) filled.
...
Follow SOLID design principles ([`architect_docs/05_DESIGN_PATTERNS_DOMAIN_SERVICES_SOLID.md`](../architect_docs/05_DESIGN_PATTERNS_DOMAIN_SERVICES_SOLID.md)). Calculations, baseline hashing, downstream tasks isolated in pure domain service classes:

```
solar_module/
└── services/
    ├── sales_order_baseline_service.py   # Compute SHA-256 baseline and audit BOM
    ├── project_spawner_service.py        # Instantiate Project container & WBS Tasks
    ├── store_logistics_service.py        # Spawn Store Manager task & manage delegation
    ├── liaisoning_inception_service.py   # Instantiate Phase 1 Liaisoning dossier
    └── sales_order_sla_service.py        # Compute 24h SLA countdown & delay validation
```
...
Frappe Desk form for `Sales Order` extended with custom section breaks, indicator badges, action buttons:

1. **Baseline Freeze Header Banner:**
   - When `custom_baseline_frozen == 1`, show teal alert box:  
...
2. **Action Button Ribbon (`frm.add_custom_button`):**
   - **"🚀 View Project WBS"** $\rightarrow$ Deep-link to `/app/project/<custom_project_reference>`.
   - **"📦 Store Logistics Desk"** $\rightarrow$ Deep-link to `/app/task/<custom_store_delivery_task>`.
   - **"⚡ Statutory Dossier"** $\rightarrow$ Deep-link to `/app/liaisoning-and-synchronization/<custom_liaisoning_reference>`.
   - **"📄 Baseline Audit Certificate"** $\rightarrow$ Render printable cryptographic audit summary.
...
Integrated in universal SPA wrapper at `/solar`:
...
1. **`Customer` Master:** Reconcile shipping and billing address IDs from Stage 05.
2. **`Project` & `Task`:** Instantiate container and tasks using pure ORM methods in atomic transaction.
3. **`Payment Schedule`:** Standard payment schedule updated to trigger downstream invoices at Stages 07, 08, 10.
4. **`Delivery Note` (Stage 07 Enabler):** `Delivery Note` creation enforces parent `Sales Order` has `docstatus == 1` and `custom_baseline_frozen == 1`.

### 7.2 Frappe CRM Touchpoints

- Sync with `tabLead` or `tabCRM Deal`, advance commercial status to **"Won / Closed - In Execution"**.
- Post formatted kickoff summary to CRM communication log.

### 7.3 Frappe HRMS Touchpoints

- Resolve `tabUser` to active `tabEmployee` to populate designated `Project Engineer`, `Store Manager`, and `Liaisoning Representative`.
...
- **Class Pattern:** Subclass `frappe.tests.utils.FrappeTestCase` or modern `frappe.testing.IntegrationTestCase`.
- **Zero-Commit Rule:** In strict accordance with Zero-Commit Rule ([`architect_docs/07_AUTOMATED_TESTING_QA_CI_CD.md`](../architect_docs/07_AUTOMATED_TESTING_QA_CI_CD.md)), tests run inside managed database transactions that roll back automatically upon teardown (`frappe.db.rollback()`). Zero calls to `frappe.db.commit()`.
...
#### SOP 1: Sales Representative / Sales Manager — Sales Order Baseline Creation & Submission

1. **Entry Trigger:** Proposal accepted in Stage 04 and Advance Payment Cleared in Stage 05 (`custom_advance_verified == 1`).
2. **Action Steps:**
   - Go to `/solar/orders` and click **"Create Sales Order from Proposal"**.
   - Select finalized `Quotation`. System auto-populates items, quantities, customer, billing address, capacity.
   - Verify line items against approved customer quote.
   - Upload scanned PDF of executed contract under **"Signed Client Contract"**.
   - Enter **"Contract Agreement Date"**.
   - Verify payment schedule terms match contract advance, dispatch, commissioning milestones.
   - Click **"Validate Baseline & Submit"**.
3. **Exit Result:** Order submitted (`docstatus = 1`), SHA-256 hash sealed, `Project` container, `Material Delivery Task`, and `Liaisoning Dossier` created.
...
2. **Action Steps:**
   - Open `/solar/orders/:id` or go to `/app/task/<store_delivery_task>`.
   - Review frozen BOM specs (module count, inverter rating, cable runs).
   - Check warehouse stock for serialized and bulk items.
   - If allocating duties to warehouse staff:
     - Click **"Delegate Task"** in action ribbon.
     - Select qualified **`Store Assistant`** from dropdown.
     - Click **"Confirm Reassignment"**.
3. **Exit Result:** Task reassigned to `Store Assistant`; staff begins serialized barcode staging for Stage 07 dispatch.
...
- **Symptom:** `Sales Order` submitted, but `custom_project_reference` or `custom_store_delivery_task` null.
- **Triage & Diagnosis:**
  1. Inspect `tabError Log` via Frappe Desk or terminal:
     ...
     ```
  2. Verify if missing foreign key or locked row prevented `Project` or `Task` creation.
- **Remediation Script:**
