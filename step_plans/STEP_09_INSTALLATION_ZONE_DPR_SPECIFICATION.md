# STEP_08_INSTALLATION_ZONE_DPR_SPECIFICATION.md
...
### 1.1 Lifecycle Positioning & The Physical Engineering Core

Stage 08 (**Zone-Based Installation Execution & Mobile DPR**) represents the core physical engineering and construction phase of the Sadbhav Solar EPC Enterprise ERP platform (`solar_module`). It bridges physical logistics fulfillment (Stage 07 Material Dispatch via Delivery Note and site Proof of Delivery sign-off) with site material reconciliation (Stage 09 Surplus Material Return to Store) and statutory electrical grid synchronization (Stage 10B Dual-Timing Liaisoning & Project Completion).

Within the **Dual Progress Bar Lifecycle Architecture** ([`docs/decisions/ADR-007-DUAL-PROGRESS-BAR-LIFECYCLE-SLA-ENGINE.md`](../docs/decisions/ADR-007-DUAL-PROGRESS-BAR-LIFECYCLE-SLA-ENGINE.md)), Stage 08 is the central operational engine of the **Project Lifecycle Stepper**. It activates immediately upon the recording of the site Proof of Delivery (POD) from Stage 07, orchestrating multi-zone site mobilization, foundation casting, structural erection, PV module installation, DC string cabling, AC inverter termination, earthing grid bonding, and pre-commissioning testing.
...
Stage 08 is engineered to replace undocumented field execution with deterministic Work Breakdown Structures, enforce strict electrical safety standards, eliminate on-site inventory leakage, and accelerate project commissioning.
...
│    Legacy: Field teams report a project is "almost finished" for three consecutive weeks while   │
│    critical AC cabling or inverter terminations have not even commenced.                         │
│    Resolution: Hierarchical WBS with physical quantity tracking (tables erected, meters pulled) │
│    mathematically rolls up actual completion percentage based on verifiable milestones.          │
│                                                                                                  │
│ 2. Unexplained Labor Budget Overruns:                                                            │
│    Legacy: Subcontractors bill for 20 workers per day while only 6 were physically on site.      │
│    Resolution: Mandatory daily DPR labor headcount by trade (masons, electricians, fitters) tied │
│    to site supervisor digital sign-off and geotagged toolbox talk photos.                         │
│                                                                                                  │
│ 3. Unaccounted Material Shrinkage & Scrap:                                                       │
│    Legacy: Hundreds of meters of 4 sq.mm solar DC cable and expensive MC4 connectors vanish from │
│    the site without record, blamed vaguely on "wastage".                                         │
│    Resolution: DPR Material Consumed ledger binds daily installed quantities against Stage 07    │
│    Delivery Notes, computing live site balances and feeding Stage 09 Surplus Reconciliation.     │
│                                                                                                  │
│ 4. Electrical Ground Faults & Inverter Fires:                                                    │
│    Legacy: Pinched cables or damaged insulation during panel installation go undetected until     │
│    grid energization, causing catastrophic inverter tripping, cable melt, or roof fire hazard.   │
│    Resolution: Strict Pre-Commissioning Electrical Verification Gate enforces 100% string Voc    │
│    checks (within ±5% tolerance) and mandatory Megger insulation testing (≥ 1.0 MΩ at 1000V DC). │
│                                                                                                  │
│ 5. Statutory CEIG & DISCOM Inspection Failures:                                                  │
│    Legacy: Government electrical inspectors reject the plant due to high earth resistance (> 5Ω) │
│    or missing test certificates, delaying net-metering and customer subsidy for months.          │
│    Resolution: Comprehensive Pre-Commissioning Test Sheet records earth pit resistance values    │
│    (≤ 5.0 Ω structure, ≤ 1.0 Ω inverter) with calibration-tagged tester documentation.           │
│                                                                                                  │
│ 6. Coordination Gaps Between Site, Store & Liaisoning:                                           │
│    Legacy: Site finishes installation but store doesn't know surplus materials are available,    │
│    and liaisoning team does not initiate net-meter application for two weeks.                    │
│    Resolution: Stage 08 completion atomically spawns Stage 09 Material Return and activates      │
│    Stage 10B Statutory Grid Synchronization countdown (10-day SLA timer).                       │
└──────────────────────────────────────────────────────────────────────────────────────────────────┘
```
...
In strict accordance with the **Zero "User" Suffix Rule** ([`step_plans/README.md#5-enterprise-persona--role-naming-standard-zero-user-suffix-rule`](./README.md#5-enterprise-persona--role-naming-standard-zero-user-suffix-rule)), all operational actors are defined by approved functional enterprise titles.
...
| **Store Assistant**                  | `Store Assistant`                  | `Store Assistant` / `Inventory Executive` | Monitors on-site material consumption against Stage 07 dispatches and prepares Stage 09 surplus return receipts.                                                   |
| **Project Manager**                  | `Project Manager`                  | `Project Delivery Head`                   | Tracks installation milestone completion, oversees project schedule against commitments, inherits all Project Engineer & Site Supervisor powers.                   |
...
| **`Project Manager`**              |    Read, Write (Milestone Sign)    | Read, Write (Oversight) |            Read Only             |       Read, Write, Submit, Sign        |            Read Only             | Audit Milestone Progress & Sign-off Completion   |
...
All relational entities strictly preserve Third Normal Form (3NF), utilize explicit field types, establish foreign key constraints with index optimizations, and follow Frappe Framework naming conventions.

### 3.1 Core DocType Extensions: `tabProject` (Installation Stage Host)

Added to ERPNext's standard `tabProject` to govern Stage 08 lifecycle progression:
...
Added to ERPNext's standard `tabTask` to bind tasks to solar zones and physical metrics:
...
Defines physical plant zones for commercial, industrial, or multi-surface residential sites.
...
The authoritative, submittable daily legal and technical record of site execution.
...
The authoritative quality verification record required before installation completion.
...
Stage 08 implements four unskippable server-side verification gates:
...
Stage 08 is governed by a **Dual SLA Clock**:

#### 1. Overall Installation Duration SLA

The target installation completion duration is calculated automatically based on total system capacity ($kW_p$) as configured in `Solar Installation Settings`:
...
- **Target Finish Datetime Calculation:**
  ...
  _(Excludes declared project holidays or force majeure weather days approved by Admin)._
- **Escalation Trigger:** If current datetime exceeds target finish datetime while status is not `Completed`:
  ...
  2. Dispatches high-priority WhatsApp and ERPNext alerts to `Project Engineer`, `Area Operations Lead`, and `Director`.
  3. Formally locks progress saving until a categorized `custom_installation_delay_reason` and mitigation plan are logged.

#### 2. Daily DPR Morning Cutoff SLA (Next-Day 10:00 AM)

- Every active site must submit its Daily Progress Report for Day $T$ before **10:00 AM on Day $T+1$**.
- A background scheduler (`solar_module.tasks.check_daily_dpr_deadlines`) executes at 10:01 AM daily:
  - Scans all projects with `custom_installation_status = 'In Progress'`.
  - Checks if a submitted DPR exists for `report_date = today - 1 day`.
  - If missing, sets `custom_dpr_submission_sla_status = 'Overdue Missing DPR'`, raises an incident flag in the Executive Command Center, and dispatches an automated reminder alert to the `Site Supervisor` and `Project Engineer`.

---

## 5. Controller Logic, Domain Services & Whitelisted APIs

In strict adherence to SOLID principles and the decoupled domain service architecture ([`architect_docs/05_DESIGN_PATTERNS_DOMAIN_SERVICES_SOLID.md`](../architect_docs/05_DESIGN_PATTERNS_DOMAIN_SERVICES_SOLID.md)), all business calculations, WBS aggregations, electrical validations, and downstream event triggers are encapsulated into pure, reusable service classes. DocType controllers remain lightweight.
...
class InstallationZoneWBSService:
    ...
    def generate_zone_wbs_tasks(cls, project_doc) -> List[str]:
        ...
        if not zones:
            # Single unified zone for compact residential rooftop installations
            ...
            # Multi-zone generation for C&I / Large Projects
            for zone in zones:
                for tmpl in cls.STANDARD_WBS_TEMPLATES:
    ...
    def aggregate_dpr_progress(cls, project_name: str) -> float:
        ...
        for t in tasks:
            ...
            # Sync task completion percentage and status
...
class PreCommissioningVerificationService:
    ...
    def validate_pre_commissioning_tests(cls, doc) -> Dict[str, Any]:
        ...
        # 1. String Electrical Tests Validation
        ...
        # 2. Earth Pit Resistance Tests Validation
        ...
        # 3. Punch List Zero-Defect Rule for Category A Items
    ...
    def execute_installation_completion_handoff(cls, project_name: str, pcomm_name: str) -> Dict[str, Any]:
        ...
        # 1. Update Project Installation Fields
        ...
        # 2. Downstream Trigger 1: Spawn Stage 09 Material Reconciliation Task
        ...
        # 3. Downstream Trigger 2: Trigger Stage 10B Statutory Liaisoning Countdown (10-Day SLA)
...
All endpoints strictly enforce `@frappe.whitelist(methods=["POST"])`, perform parameter type validation, assert in-method document permissions, and prevent Insecure Direct Object References (IDOR).
...
def initialize_project_installation(project_name: str) -> Dict[str, Any]:
    ...
    # Assert Delivery Note POD was recorded
...
def get_site_material_balance(project_name: str) -> Dict[str, Any]:
    ...
    # Aggregate total dispatched from submitted Delivery Notes
    ...
    # Aggregate installed from submitted DPRs
...
All frontend interactions operate under the **Unified Frontend Landing Architecture** ([`planning_ref_docs/README.prd.md#universal-frontend-landing-architecture`](../planning_ref_docs/README.prd.md#universal-frontend-landing-architecture)), delivering an intuitive mobile experience on `/solar` (Vue 3 + Frappe UI SPA) while preserving permission-gated deep links to Frappe Desk forms.
...
│                                      │ flows into the digital twin record for lifetime O&M.      │
...
Every unit and integration test executes within modern `frappe.testing.IntegrationTestCase` under the **Zero-Commit Rule** (`frappe.db.rollback()`), guaranteeing zero database side effects.
...
class TestStage08Installation(IntegrationTestCase):
    ...
    def test_wbs_task_generation(self):
        ...
        # Assert specific task types exist
    ...
    def test_dpr_submission_updates_wbs_progress(self):
        ...
        # Assert project cumulative progress updated
            ...
            "string_electrical_tests": [
                {
                    ...
                    "measured_isc": 13.5,
                    "polarity_verified": 1,
                    "insulation_resistance_pos": 0.4,  # Failed Megger!
                    "insulation_resistance_neg": 15.0,
                }
            ],
            "earth_pit_tests": [
            ...
            "string_electrical_tests": [
                {
                    ...
                    "module_count_in_string": 18,
                    "theoretical_design_voc": 800.0,
                    "measured_voc": 802.0,  # Within ±5%
                    "measured_isc": 13.5,
                    "polarity_verified": 1,
                    "insulation_resistance_pos": 20.0,  # > 1.0 MΩ
                    "insulation_resistance_neg": 22.0,  # > 1.0 MΩ
                }
            ],
            "earth_pit_tests": [
...
1. Log in to the `/solar` portal on smartphone or tablet.
2. Navigate to **Active Projects** $\rightarrow$ Select assigned project $\rightarrow$ Click **"Mobilize Site Execution"**.
3. System verifies that Stage 07 Delivery Note POD is confirmed on site.
4. Conduct the mandatory morning safety toolbox briefing with all workers. Capture a geotagged photo of the crew in full PPE and submit the mobilization checklist.

#### Step 2: Daily Progress Report (DPR) Entry

1. At the end of each working shift (or progressively during the day), open **Mobile DPR Entry**.
2. Select current shift (`Day Shift` / `Night Shift`), current weather condition, and log any hours lost to adverse weather.
3. Record the physical labor headcount across trades: Civil Masons, Structural Fitters, Certified Electricians, and Helpers.
4. Input the physical units executed today for each milestone (e.g. 15 pedestals cast, 40 panels mounted, 120m DC cable routed).
5. In the **Material Consumed** tab, record the specific quantities installed today. System displays live remaining balances on site.
6. Upload at least two mandatory milestone photos (e.g. MMS torque check, string alignment).
7. Click **"Submit Daily Progress Report"**. System rolls up physical progress percentage into the Project Stepper.

#### Step 3: Pre-Commissioning Electrical Testing & Handover

1. Once 100% of WBS tasks reach completion, the Project Stepper displays **"Initiate Pre-Commissioning Testing"**.
2. Field engineer and QC specialist connect calibrated multimeters and Megger insulation testers.
3. Record string voltage ($V_{oc}$), operating current ($I_{sc}$), polarity verification, and insulation resistance ($\ge 1.0\text{ M}\Omega$) for every DC string.
4. Measure and log earth pit resistance values ($\le 5.0\ \Omega$ for structure, $\le 1.0\ \Omega$ for inverters).
5. Ensure all Category A punch list items are marked `Verified Closed`.
6. Submit the pre-commissioning record with `Passed` verdict. System automatically marks installation complete, spawns Stage 09 Material Return, and triggers Stage 10B statutory grid sync.
...
- **Symptom:** Automated 10:00 AM DPR cutoff alerts are not firing for overdue sites.
- **Triage Steps:**
  1. Inspect the Redis worker queues:
     ```bash
     bench --site <site-name> doctor
     ```
  2. Verify that the scheduled event `solar_module.tasks.check_daily_dpr_deadlines` is enabled in `hooks.py`:
     ```python
     # hooks.py
     scheduler_events = {
         "cron": {
     ...
     ```
  3. Execute manual trigger via Bench console:
     ```python
...
- **Symptom:** DPR submitted successfully but `custom_cumulative_dpr_completion_pct` on `Project` remains at 0%.
- **Remediation Script:**
  ```python
...
A Stage 08 implementation is strictly incomplete until every item on this checklist is verified:
