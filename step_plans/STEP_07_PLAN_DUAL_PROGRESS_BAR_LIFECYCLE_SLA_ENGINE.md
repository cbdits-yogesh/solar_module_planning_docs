...
| **Lead Qualification Officer**      | **`Lead Representative`**         | `Inside Sales Executive`                  | Stage 01 Lead ingestion, qualification, survey booking.          | Read/Write Lead; trigger survey schedule drawer.                    |
| **Site Survey Specialist**          | **`Survey Engineer`**             | `Field Survey Auditor`                    | Stage 02 technical site survey, GPS audit, 6 photos.             | Read/Write Survey; mobile audit launch; offline sync.               |
| **PV CAD & Electrical Designer**    | **`Design Engineer`**             | `Solar Design Engineer`                   | Stage 03 PV system layout, SLD, dynamic BOM explosion.           | Read/Write Design; CAD/SLD upload drawer; BOM freeze.               |
| **Commercial & Proposal Officer**   | **`Sales Representative`**        | `Commercial Sales Executive`              | Stage 04 quotation proposal, tariff analysis, subsidy.           | Read/Write Quotation; proposal generator; contract dispatch.        |
| **Commercial Operations Lead**      | **`Commercial Officer`**          | `Sales Operations Executive`              | Stage 06 Sales Order baseline verification & submission.         | Read/Write/Submit Sales Order; baseline hash freeze.                |
| **Finance & Accounts Officer**      | **`Accounts Assistant`**          | `Accounts Executive`                      | Stage 05 customer advance verification, bank UTR check.          | Read/Write Payment Entry; advance clearance gate sign-off.          |
| **Warehouse Store Lead**            | **`Store Manager`**               | `Warehouse Supervisor`                    | Stage 07 dispatch staging, stock reservation, delegation.        | Read/Write/Submit Delivery Note; reassign store task.               |
| **Warehouse Fulfillment Assistant** | **`Store Assistant`**             | `Store Assistant` / `Inventory Executive` | Stage 07 serialized picking, barcode scanning; Stage 09 returns. | Read/Write Delivery Note, Stock Entry (Return).                     |
| **Field Execution Lead**            | **`Project Engineer`**            | `Site Execution Engineer`                 | Stage 08 multi-zone installation, daily progress reports (DPR).  | Read/Write Project & WBS Tasks; submit DPR drawer.                  |
| **Statutory Compliance Officer**    | **`Liaisoning Officer`**          | `Statutory Compliance Executive`          | Stage 10A early DISCOM filing; Stage 10B grid synchronization.   | Read/Write/Submit Liaisoning Dossier; confirm grid sync.            |
...
| `custom_completion_certified_by` | Completed Certified By      | `Link`     | `User`                                                                                                           |    No     |   -   | `Liaisoning Officer` or `Admin` who verified grid synchronization.   |
...
2. **Actor Badge:** Displays assigned user name, avatar, Frappe HRMS designation (`Lead Representative`, `Survey Engineer`, `Design Engineer`, etc.), and contact phone.
...
3. **Project Terminal State (`custom_is_completed_flag = 1`, `status = 'Completed'`):**
   - Triggered strictly by `Liaisoning Officer` upon completing Stage 10B (`phase_2_status = 'Grid Synchronized'`).
