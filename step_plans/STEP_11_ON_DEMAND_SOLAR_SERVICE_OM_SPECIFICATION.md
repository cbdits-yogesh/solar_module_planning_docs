...
Stage 11 (**On-Demand Solar Service, Incident-Driven O&M & Warranty Lifecycle**) constitutes the post-commissioning, after-sales operational lifecycle of the Sadbhav Solar EPC Enterprise ERP platform (`solar_module`).

In strict alignment with the enterprise governance model established in [`ADR-011`](../docs/decisions/ADR-011-ON-DEMAND-SOLAR-SERVICE-WARRANTY-OM-LIFECYCLE.md):

1. **Clean CapEx Project Termination:** Stage 10 (**Dual-Timing Statutory Liaisoning, Regulatory Inspection, Net Metering & Grid Synchronization**) represents the authoritative legal, technical, and commercial closeout of the solar EPC project. The capital expenditure (CapEx) construction project terminates cleanly at Stage 10 with net-meter energization, Commercial Operation Date (COD) certification, and final milestone revenue recognition.
2. **Independent, Decoupled OpEx Lifecycle:** Stage 11 is **not** automatically instantiated upon project closeout. It operates completely independently of upstream construction steps. It creates zero automatic tasks, tickets, or dependencies on the completed `tabProject` record.
3. **On-Demand Incident Activation:** Stage 11 activates exclusively on-demand when an operational solar plant experiences an inverter failure, generation drop, electrical tripping, physical component damage, or when a customer explicitly requests a preventative health check or routine module cleaning.
4. **Dual-Track Commercial Governance:**
   - **Track A (Within-Warranty / Free / RMA):** Equipment or installation workmanship under active warranty coverage $\rightarrow$ ₹0.00 customer charge, automated OEM reverse logistics / RMA warranty claim processing with component manufacturers.
   - **Track B (Out-of-Warranty / Chargeable):** Expired warranty, physical impact, pest damage, grid surges, or voluntary maintenance $\rightarrow$ service fee estimation, spare parts pricing, customer quotation approval, payment collection, and ERPNext Sales Invoice generation.
...
Stage 11 enforces customer retention, operational velocity, and financial protection:

| Strategic Objective             | Metric / Target               | Operational Mechanism                                                                                                  |
| :------------------------------ | :---------------------------- | :--------------------------------------------------------------------------------------------------------------------- |
| **First-Time Fix Rate (FTFR)**  | $\ge 85\%$                    | Accurate omnichannel intake capturing error codes and photos, enabling technicians to carry correct replacement parts. |
| **Emergency Triage Velocity**   | $\le 4\text{ Hours}$          | Automated priority triage for complete plant blackouts with immediate dispatcher and engineer mobile paging.           |
| **Warranty Leakage Prevention** | $100\%$ zero unbilled repairs | Automated warranty discrimination blocking out-of-warranty visits from proceeding without approved estimates.          |
| **OEM Warranty Cost Recovery**  | $\ge 95\%$ recovery rate      | Automated logging of removed defective serial numbers into OEM RMA tracking for manufacturer credit.                   |
| **Field Visit Authenticity**    | $100\%$ geofenced visits      | Mandatory device GPS lock matching registered site coordinates within 500 meters before diagnostic form unlock.        |
| **Asset Twin Integrity**        | $100\%$ serialized tracking   | Compulsory tracking of both removed and newly installed serial numbers, updating the permanent site twin.              |
...
| **Architecture Decision**    | [`docs/decisions/ADR-011-ON-DEMAND-SOLAR-SERVICE-WARRANTY-OM-LIFECYCLE.md`](../docs/decisions/ADR-011-ON-DEMAND-SOLAR-SERVICE-WARRANTY-OM-LIFECYCLE.md) | Governs decoupled post-project service boundary, dual-track warranty logic, GPS geofencing, and site service ledger.         |
| **Project Foundation Model** | [`planning_ref_docs/01_PROJECT_FOUNDATION_MODEL.md`](../planning_ref_docs/01_PROJECT_FOUNDATION_MODEL.md) (`BC-11`, `Sec 3.11`)                         | Incident intake, field technician dispatch, component replacement, and maintenance visit closure.                            |
| **Business Requirements**    | [`planning_ref_docs/05_BUSINESS_REQUIREMENTS_DOCUMENT.md`](../planning_ref_docs/05_BUSINESS_REQUIREMENTS_DOCUMENT.md) (`BR-011`)                        | Omnichannel booking, warranty vs chargeable repair segregation, and permanent plant service history.                         |
| **Functional Requirements**  | [`planning_ref_docs/06_FUNCTIONAL_REQUIREMENTS_SPECIFICATION.md`](../planning_ref_docs/06_FUNCTIONAL_REQUIREMENTS_SPECIFICATION.md) (`FR-011`)          | Service request portal, mobile technician check-in, spare parts reconciliation, customer OTP, and invoice generation.        |
| **Software Requirements**    | [`planning_ref_docs/07_SOFTWARE_REQUIREMENTS_SPECIFICATION.md`](../planning_ref_docs/07_SOFTWARE_REQUIREMENTS_SPECIFICATION.md) (`Sec 2.2`, `Sec 4.4`)  | Background SLA monitor daemon, WhatsApp communication broker, and ERPNext Stock/Accounts bridge.                             |
| **Database Design**          | [`planning_ref_docs/08_DATABASE_DESIGN_DOCUMENT.md`](../planning_ref_docs/08_DATABASE_DESIGN_DOCUMENT.md) (`Domain 9: OM`)                              | Relational schemas for `tabSolar Service Request`, `tabMaintenance Visit`, and child tables.                                 |
| **API Architecture**         | [`planning_ref_docs/09_API_DESIGN_AND_INTEGRATIONS.md`](../planning_ref_docs/09_API_DESIGN_AND_INTEGRATIONS.md) (`API 10`)                              | Whitelisted RPC endpoints: `book_service_request`, `triage_and_dispatch`, `technician_checkin`, `submit_service_resolution`. |
| **UI/UX Specification**      | [`planning_ref_docs/10_UI_UX_SPECIFICATION.md`](../planning_ref_docs/10_UI_UX_SPECIFICATION.md) (`Screen 15`)                                           | Customer Service Booking Widget, O&M Dispatch Workbench, and Mobile Field Service UI.                                        |

---

### 1.4 Critical Operational Failures Eliminated

1. **Indefinite CapEx Project Linger:** Prevents EPC projects from remaining open for years under the guise of "warranty monitoring", ensuring clean financial audit boundaries.
2. **Uncollected Out-of-Warranty Revenue:** Eliminates the practice of technicians performing free repairs for customer-inflicted damages (e.g. severed AC cables from terrace renovation) by enforcing upfront quotations.
3. **Unrecovered OEM Inverter & Module Costs:** Stops contractors from writing off defective inverters out of pocket by capturing defective serial numbers for formal OEM supplier RMA reimbursement.
4. **Phantom Field Service Visits:** Eliminates technician falsification of site visits through mandatory Haversine GPS geofencing.
5. **Corrupted Plant Digital Twins:** Prevents the loss of equipment serial tracking by strictly coupling part consumption with removed vs. installed serial numbers.
...
In strict accordance with the **Zero "User" Suffix Rule**, all actors are defined using professional enterprise titles:
...
- **Name:** `Solar Service Request`
- **Table Name:** `tabSolar Service Request`
...
- **Parent:** `tabMaintenance Visit`
- **Parentfield:** `spare_items`
- **Submittable:** Inherits from parent
...
- **Name:** `Solar Site Service History`
- **Table Name:** `tabSolar Site Service History`
- **Submittable:** No (Append-only ledger managed by system domain services)
- **Autoname:** `SSH-.#####`
...
- **Parent:** `tabSolar Service Request` / `tabMaintenance Visit`
- **Parentfield:** `delay_logs`
...
- **Enforcement Point:** `Solar Service Request.validate()` and triage transition.
- **Rule:** Automatic server-side check. If `commissioning_date + 365 days >= today` and fault is equipment-related, default `warranty_status = "Under Warranty"` and `is_free_service = 1`. If past warranty or caused by external force (cable chewed by rodents, lightning strike, external water damage), system mandates `warranty_status = "Out of Warranty"`, sets `is_free_service = 0`, and prevents dispatch until `customer_estimate_accepted = 1`. Non-Admin cannot override warranty to "Free" without logging justification.

#### Gate 2: Field Check-In Geofence Gate

- **Enforcement Point:** `solar_module.api.om.technician_checkin` and `Maintenance Visit.validate()`.
- **Rule:** The field technician cannot view or submit the diagnostic checklist without submitting GPS coordinates matching the registered installation site within $500\text{ meters}$, calculated using the Haversine formula:
...
- **Enforcement Point:** `Maintenance Visit.on_submit()`.
- **Rule:** For any child row in `tabSolar Service Spare Item` marked `is_serialized = 1`, both `serial_no_removed` and `serial_no_installed` are compulsory.
  1. `serial_no_removed` must exist in the active site asset record and is automatically unlinked and moved to `Quarantine / RMA Warehouse`.
  2. `serial_no_installed` must exist in `Available` status in the technician's mobile van stock (`Van Warehouse - SEPC`) and is automatically moved to the site ledger.
  3. Violations raise `frappe.ValidationError(_("Serialized asset replacement requires both Removed Serial No and Installed Serial No."))`.

#### Gate 4: Customer Closed-Loop Gate

- **Enforcement Point:** `Maintenance Visit.on_submit()`.
- **Rule:** Final document submission (`docstatus = 1`) is strictly blocked unless either:
  - `is_otp_verified = 1` (valid 6-digit OTP verified against customer mobile phone via SMS/WhatsApp), OR
  - `customer_signature` is non-empty and `signee_name` has length $\ge 3$ characters.

---

### 4.3 Tiered Service SLAs & Delay Enforcement

The background daemon `solar_module.tasks.check_service_slas` executes every 15 minutes:

| Priority Class         | Response SLA      | Resolution SLA           | Escalation Pathway                                                                                |
| :--------------------- | :---------------- | :----------------------- | :------------------------------------------------------------------------------------------------ |
| **Critical Emergency** | $4\text{ Hours}$  | $24\text{ Hours}$        | T-1h WhatsApp alert to Coordinator $\rightarrow$ Breached: Immediate alert to O&M Head & `Admin`. |
| **High / Degraded**    | $12\text{ Hours}$ | $48\text{ Hours}$        | T-2h alert to Coordinator $\rightarrow$ Breached: Paged to O&M Service Coordinator.               |
| **Standard / Routine** | $48\text{ Hours}$ | $5\text{ Business Days}$ | T-8h reminder $\rightarrow$ Breached: Notification in daily digest.                               |

**Mandatory Delay Logging:** When a ticket breaches its response or resolution SLA, its status shifts to `Overdue`. Submitting resolution for an overdue ticket is hard-blocked unless at least one entry exists in `tabSolar Stage Delay Log` explaining the breach.
...
def generate_customer_otp(visit_name: str):
    ...
    # Broadcast to customer mobile
    phone = frappe.db.get_value("Customer", visit.customer, "custom_contact_phone")
    # Dispatched via solar_module.notifications.broker
...
Following the platform routing contract, users access the O&M application at `/solar/service`:
...
1. **ERPNext Stock (`Stock Entry - Material Issue`):**
   - Consuming spare parts automatically generates a `Stock Entry` (Purpose: Material Issue), transferring components out of `Technician Van Warehouse` or `Central Store`.
2. **ERPNext Accounts (`Sales Invoice`):**
   - For out-of-warranty services, submitting `Maintenance Visit` automatically triggers an ERPNext `Sales Invoice` against the `Customer`, billing labor charges and non-warranty components.
3. **Frappe HRMS (`Employee Checkin`):**
   - Geofenced check-in writes to `Employee Checkin` for the technician, automatically logging remote field attendance.
4. **Omnichannel Communications (WhatsApp / SMS):**
   - Automated message to customer: "Your service request #SSR-2026-00102 is assigned to Engineer Rahul Sharma. ETA: 11:30 AM."
   - Automated OTP message for job completion sign-off.
...
Located at `solar_module/tests/test_solar_service_om.py`, inheriting from `frappe.tests.utils.FrappeTestCase`:
...
1. **Intake Screening:**
   - Inquire whether the plant is completely shut down (Emergency) or partially generating (Degraded).
   - Request customer to read the LCD/LED screen on the solar inverter or capture a photo of the error code.
2. **Warranty Check:**
   - Verify COD date and installation records.
   - If within 1 year, inform customer the diagnostic visit is covered under contractor warranty.
   - If out of warranty, notify customer of the standard ₹1,500 inspection visit fee and obtain verbal/digital acceptance.

---

### 9.2 Field Technician Diagnostics & Safety SOP (LOTO)

1. **Electrical Safety & PPE:**
   - Wear Class 0 (1000V rated) electrical gloves, safety goggles, and rubber-soled boots.
2. **Lockout/Tagout (LOTO):**
   - Switch off the AC isolator first, followed by the DC isolator. Tag the breaker before touching any exposed terminals.
3. **Diagnostics Protocol:**
   - Measure string open-circuit voltage ($V_{oc}$) on DC side. Assert within $\pm 5\%$ of rated nameplate calculation.
   - Test insulation resistance with Megger ($\ge 1.0\text{ M}\Omega$).
   - Verify inverter error code against manufacturer service handbook.
4. **Parts Replacement & Sign-Off:**
   - Scan barcode of damaged unit removed.
   - Scan barcode of new replacement unit installed.
   - Re-energize plant and verify active grid generation ($kW > 0$).
   - Generate and verify customer OTP before vacating the site.
...
4. Upon payment clearance, dispatch status unlocks automatically.
...
- **GPS Distance Mismatch Troubleshooting:** If a customer roof has incorrect coordinates in the master, `Admin` can update site coordinates via `/app/customer/<id>` using Google Maps satellite pin-drop.
- **SLA Daemon Health Check:** Check Redis queue status:
  ```bash
  bench execute solar_module.tasks.check_service_slas
  ```
- **Service Ledger Audit:** Query permanent maintenance events:
  ```sql
