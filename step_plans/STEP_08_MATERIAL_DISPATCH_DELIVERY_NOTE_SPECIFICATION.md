# STEP_07_MATERIAL_DISPATCH_DELIVERY_NOTE_SPECIFICATION.md
...
Stage 07 (**Material Dispatch Logistics via Delivery Note**) represents the critical physical transition from central warehouse inventory to site execution in the Sadbhav Solar EPC Enterprise ERP platform (`solar_module`). It bridges the commercial contractual baseline (Stage 06 Sales Order Baseline Freeze) with on-site physical engineering execution (Stage 08 Zone-Based Installation & Mobile DPR).

Within the **Dual Progress Bar Lifecycle Architecture** ([`docs/decisions/ADR-007-DUAL-PROGRESS-BAR-LIFECYCLE-SLA-ENGINE.md`](../docs/decisions/ADR-007-DUAL-PROGRESS-BAR-LIFECYCLE-SLA-ENGINE.md)), Stage 07 is the first active physical execution stage on the **Project Lifecycle Stepper**, triggered immediately upon the submission of the contractually frozen `Sales Order`.
...
Stage 07 is engineered to eliminate warehouse bottlenecks, prevent inventory leakage, ensure full statutory compliance with Indian GST transport laws, and establish unbreakable equipment provenance.

| Metric / KPI Code                 | Target Objective                                  | Legacy Baseline                           | Target Standard        | Operational Impact                                                              |
| :-------------------------------- | :------------------------------------------------ | :---------------------------------------- | :--------------------- | :------------------------------------------------------------------------------ |
| **KPI-01: Serial Integrity**      | 100% 2D Barcode Scan for Modules & Inverters      | $\approx 20\%$ (Manual paper notes)       | **100.0% Strict Gate** | Zero OEM warranty claim rejections; perfect Stage 11 Asset Register sync.       |
| **KPI-02: Dispatch TAT**          | Warehouse Dispatch Turnaround Time                | 5 to 9 Days                               | **$\le$ 48 Hours**     | Rapid mobilization; site labor teams never stand idle waiting for materials.    |
| **KPI-03: E-Way Bill Compliance** | Valid E-Way Bill on all consignments $\ge$ ₹50k   | $\approx 85\%$ (Informal local transport) | **100.0% Hard Block**  | Zero highway seizures, vehicle impoundments, or GST Section 129 penalties.      |
| **KPI-04: Over-Dispatch Control** | Over-dispatch against frozen SO Bill of Materials | 8% to 14% unaccounted leakage             | **0.0% Unauthorized**  | Eliminates unaccounted inventory shrinkage; preserves project gross margins.    |
| **KPI-05: POD Loop Closure**      | Digital Proof of Delivery with site signature     | $\approx 15\%$ (Lost paper receipts)      | **100.0% Digital POD** | Instant dispute resolution; verifies goods received in sound condition on site. |
...
| **UI/UX Specifications**          | [`planning_ref_docs/10_UI_UX_SPECIFICATION.md`](../planning_ref_docs/10_UI_UX_SPECIFICATION.md)                                     | Screen 10: Store Dispatch Desk, Quick Barcode Scanner & Mobile POD Interface.                |
| **Standard Operating Procedure**  | [`planning_ref_docs/11_MODULE_FUNCTIONAL_DOCUMENTATION.md`](../planning_ref_docs/11_MODULE_FUNCTIONAL_DOCUMENTATION.md)             | `MOD-07`: Authorized physical dispatch of materials from central store to installation site. |
...
│    Legacy: Modules damaged after 2 years rejected by tier-1 manufacturer because serial numbers  │
│    were never mapped to the original project supply contract.                                    │
│    Resolution: Strict 100% 2D barcode scan creates an immutable Serial and Batch Bundle record.  │
│                                                                                                  │
│ 2. Highway Seizure & GST Penalties:                                                              │
│    Legacy: Vehicles impounded under CGST Act Sec 129 due to expired or missing E-Way Bills.      │
│    Resolution: System blocks Delivery Note submission for consignments ≥ ₹50,000 unless a valid  │
│    12-digit E-Way Bill Number, validity datetime, and PDF attachment are verified.              │
│                                                                                                  │
│ 3. Uncontrolled Material Leakage (Margin Slippage):                                              │
│    Legacy: Storekeepers dishing out extra cable drums or structure channels on informal phone calls. │
│    Resolution: Real-time mathematical ceiling blocks dispatch exceeding frozen Sales Order BOM.  │
│                                                                                                  │
│ 4. Site Team Idle Standing Costs:                                                                │
│    Legacy: Installation teams mobilize to site only to discover structures or panels never left. │
│    Resolution: Automated 48h Store Dispatch SLA engine with warning escalations at 36h.          │
│                                                                                                  │
│ 5. Transit Damage Finger-Pointing:                                                               │
│    Legacy: Site supervisor claims inverters arrived shattered; store claims they left intact.    │
│    Resolution: Two-tier photographic gate: Pre-dispatch checklist at store + Mobile POD at site.│
└──────────────────────────────────────────────────────────────────────────────────────────────────┘
```
...
In strict accordance with the **Zero "User" Suffix Rule** ([`step_plans/README.md#5-enterprise-persona--role-naming-standard-zero-user-suffix-rule`](./README.md#5-enterprise-persona--role-naming-standard-zero-user-suffix-rule)), all operational actors are defined by functional titles.
...
| :----------------------- | :----------------- | :---------------------------------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Store Manager**        | `Store Manager`    | `Store Manager` / `Warehouse In-Charge`   | Apex warehouse authority. Receives material dispatch task from Stage 06, assigns/delegates tasks to Store Assistant, validates picklists, authorizes release, reviews SLA timers, oversees transport. |
| **Store Assistant**      | `Store Assistant`  | `Store Assistant` / `Inventory Executive` | Frontline warehouse operator. Executes physical picking, 2D barcode scanning, pre-dispatch quality checks, enters vehicle/driver manifest, attaches statutory E-Way Bills.                            |
| **Project Engineer**     | `Project Engineer` | `Project Engineer` / `Field Engineer`     | Site technical lead. Destination recipient. Conducts on-site unloading inspection, verifies serial counts, signs digital POD on mobile portal.                                                        |
| **Site Supervisor**      | `Site Supervisor`  | `Site Supervisor`                         | Operational field supervisor. Assists in receiving materials at site, verifies physical carton integrity, signs delivery receipt.                                                                     |
| **Project Manager**      | `Project Manager`  | `Project Delivery Head`                   | Oversees project milestones, correlates dispatch completion with site readiness and customer billing milestones.                                                                                      |
| **Admin**                | `Admin`            | `Director` / `Head of Operations`         | Project-level Supreme Command. Authorizes over-dispatch waivers, emergency dispatch overrides, SLA extensions, and customizes `Solar Dispatch Settings`.                                              |
| **System Manager**       | `System Manager`   | `Lead Software Architect` / `DevOps`      | Framework Supreme. Configures DocType schemas, background RQ queues, and API integrations.                                                                                                            |
...
| **`Project Manager`**  |                 Read Only                  |          Read Only           |          Read Only           |       Read Only        | Monitor Order Delivery & Site Readiness                           |
...
Stage 07 extends ERPNext core DocTypes (`tabDelivery Note`, `tabDelivery Note Item`, `tabTask`) via fixtures and defines a custom child table and single settings DocType in `solar_module`.
...
| :------------------------------ | :------------------------------ | :----------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---------: | :------: | :------------------------------------------------------------------------------------------------------------------------------------ |
| `custom_is_solar_dispatch`      | Is Solar EPC Dispatch           | `Check`      | -                                                                                                                                                                                          |     Yes     | Index: 1 | Flag distinguishing solar dispatches; triggers serial validation & gates. Default: 1.                                                 |
| `custom_sales_order_ref`        | Solar Sales Order               | `Link`       | `Sales Order`                                                                                                                                                                              |     Yes     | Index: 1 | Originating commercial contract. Must have `docstatus == 1` and `custom_baseline_frozen == 1`.                                        |
| `custom_project_ref`            | Solar Project                   | `Link`       | `Project`                                                                                                                                                                                  |     Yes     | Index: 1 | Destination project container. Auto-fetched from Sales Order.                                                                         |
| `custom_store_task_ref`         | Store Dispatch Task             | `Link`       | `Task`                                                                                                                                                                                     |     No      | Index: 1 | Reverse link to Store Delivery task spawned in Stage 06. Auto-marked Completed upon POD.                                              |
| `custom_lead_ref`               | Originating Lead                | `Link`       | `Lead`                                                                                                                                                                                     |     Yes     | Index: 1 | Preserves lead identity thread across logistics and installation.                                                                     |
| `custom_dispatch_stage`         | Dispatch Consignment Stage      | `Select`     | `Phase 1 - Civil & Structure\nPhase 2 - Solar PV Modules\nPhase 3 - Inverters & BOS\nPhase 4 - Complete Single Dispatch\nAd-hoc / Balance Dispatch`                                        |     Yes     | Index: 1 | Categorizes consignment phase for site readiness alignment.                                                                           |
| `custom_transporter_name`       | Transporter Name                | `Data`       | -                                                                                                                                                                                          |     Yes     |    -     | Name of transport / logistics carrier firm.                                                                                           |
| `custom_transporter_gstin`      | Transporter GSTIN               | `Data`       | -                                                                                                                                                                                          | Conditional |    -     | 15-character Indian GSTIN regex: `^[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z]{1}[1-9A-Z]{1}Z[0-9A-Z]{1}$`. Mandatory if Grand Total $\ge$ ₹50,000. |
| `custom_vehicle_no`             | Transport Vehicle Registration  | `Data`       | -                                                                                                                                                                                          |     Yes     |    -     | Vehicle license plate. Uppercase regex: `^[A-Z]{2}[0-9]{1,2}[A-Z]{1,3}[0-9]{4}$`.                                                     |
| `custom_driver_name`            | Driver Full Name                | `Data`       | -                                                                                                                                                                                          |     Yes     |    -     | Name of commercial driver operating transport vehicle.                                                                                |
| `custom_driver_phone`           | Driver Contact Mobile           | `Data`       | -                                                                                                                                                                                          |     Yes     |    -     | 10-digit Indian mobile regex: `^[6-9][0-9]{9}$`. Real-time SMS dispatch destination.                                                  |
| `custom_lr_number`              | Lorry Receipt (LR / Bilty) No   | `Data`       | -                                                                                                                                                                                          |     No      |    -     | Transporter consignment note number.                                                                                                  |
| `custom_lr_date`                | LR / Bilty Date                 | `Date`       | -                                                                                                                                                                                          |     No      |    -     | Date of freight note issuance.                                                                                                        |
| `custom_eway_bill_no`           | E-Way Bill Number               | `Data`       | -                                                                                                                                                                                          | Conditional | Index: 1 | 12-digit Indian GST E-Way Bill regex: `^[0-9]{12}$`. Mandatory if consignment $\ge$ ₹50,000.                                          |
| `custom_eway_bill_date`         | E-Way Bill Date                 | `Datetime`   | -                                                                                                                                                                                          | Conditional |    -     | Timestamp of E-Way Bill generation on the GST portal.                                                                                 |
| `custom_eway_bill_validity`     | E-Way Bill Expiry Datetime      | `Datetime`   | -                                                                                                                                                                                          | Conditional |    -     | Timestamp when transit validity expires. Must be $\ge$ posting_datetime.                                                              |
| `custom_eway_bill_doc`          | E-Way Bill Official PDF         | `Attach`     | -                                                                                                                                                                                          | Conditional |    -     | Scanned / downloaded PDF of official E-Way Bill. Mandatory if $\ge$ ₹50,000.                                                          |
| `custom_site_delivery_address`  | Site Delivery Address           | `Link`       | `Address`                                                                                                                                                                                  |     Yes     |    -     | Physical installation site address where goods are unloaded.                                                                          |
| `custom_site_contact_person`    | Site Receiving Engineer         | `Data`       | -                                                                                                                                                                                          |     Yes     |    -     | Name of Site Supervisor or Project Engineer at destination.                                                                           |
| `custom_site_contact_phone`     | Site Contact Phone              | `Data`       | -                                                                                                                                                                                          |     Yes     |    -     | 10-digit phone of on-site receiving engineer.                                                                                         |
| `custom_pre_inspection_passed`  | Pre-Dispatch Inspection Passed  | `Check`      | -                                                                                                                                                                                          |     Yes     |    -     | System flag: 1 only when all mandatory inspection checklist rows are Passed.                                                          |
| `custom_pod_status`             | Proof of Delivery Status        | `Select`     | `Pending In-Transit\nDelivered at Site\nPartially Received with Shortage\nDamaged in Transit\nReturned to Store`                                                                           |     Yes     | Index: 1 | Operational status of physical site handover. Default: `Pending In-Transit`.                                                          |
| `custom_pod_received_by`        | POD Confirmed By                | `Data`       | -                                                                                                                                                                                          |     No      |    -     | Full name of engineer signing POD on site.                                                                                            |
| `custom_pod_received_on`        | POD Received Datetime           | `Datetime`   | -                                                                                                                                                                                          |     No      |    -     | Timestamp when POD was signed at site.                                                                                                |
| `custom_pod_signature_doc`      | Digital Signature Image         | `Attach`     | -                                                                                                                                                                                          |     No      |    -     | PNG/SVG signature captured on `/solar` mobile portal.                                                                                 |
| `custom_pod_photos_doc`         | Unloading Photographic Evidence | `Attach`     | -                                                                                                                                                                                          |     No      |    -     | Photo of materials safely unloaded at project site.                                                                                   |
| `custom_pod_remarks`            | POD Delivery Remarks            | `Small Text` | -                                                                                                                                                                                          |     No      |    -     | Site inspection remarks, shortages noted, or package condition notes.                                                                 |
| `custom_dispatch_sla_hours`     | Target Dispatch SLA Hours       | `Int`        | -                                                                                                                                                                                          |     Yes     |    -     | Target duration from Store Task creation to DN submission. Default: 48.                                                               |
| `custom_sla_status`             | SLA Clock Status                | `Select`     | `Within SLA\nWarning (< 12h)\nOverdue\nMet SLA\nBreached SLA`                                                                                                                              |     Yes     | Index: 1 | Real-time SLA tracker state.                                                                                                          |
| `custom_dispatch_delay_reason`  | Delay Reason Category           | `Select`     | `Stock Shortage from Supplier\nTransport / Vehicle Delay\nE-Way Bill Portal Failure\nSite Civil Foundation Incomplete\nCustomer Payment / Hold Request\nAdverse Weather Conditions\nOther` |     No      |    -     | Mandatory if DN is submitted with `custom_sla_status == 'Overdue'`.                                                                   |
| `custom_dispatch_delay_remarks` | Delay Detailed Justification    | `Small Text` | -                                                                                                                                                                                          |     No      |    -     | Mandatory detailed explanation if submitted past 48h SLA threshold.                                                                   |
...
| Fieldname                         | Label                       | Fieldtype | Options / Target                                                                                                               | Mandatory |  Index   | Description & Validation Invariant                                                        |
| :-------------------------------- | :-------------------------- | :-------- | :----------------------------------------------------------------------------------------------------------------------------- | :-------: | :------: | :---------------------------------------------------------------------------------------- |
| `custom_item_category`            | Solar Component Category    | `Select`  | `Solar Module\nSolar Inverter\nStructure MMS\nDC Cable\nAC Cable\nCombiner Box / AJB\nEarthing & Lightning\nBalance of System` |    Yes    | Index: 1 | Component class. Drives serialized vs batched vs bulk validation rules.                   |
| `custom_is_serialized`            | Serial Number Required      | `Check`   | -                                                                                                                              |    Yes    |    -     | Fetched from Item master (`has_serial_no`). Mandatory for Modules & Inverters.            |
| `custom_so_ordered_qty`           | Sales Order Baseline Qty    | `Float`   | -                                                                                                                              |    Yes    |    -     | Contractually frozen item quantity from Stage 06 Sales Order.                             |
| `custom_previously_disp_qty`      | Previously Dispatched Qty   | `Float`   | -                                                                                                                              |    Yes    |    -     | Cumulative quantity already dispatched across prior submitted Delivery Notes.             |
| `custom_balance_so_qty`           | Remaining Un-dispatched Qty | `Float`   | -                                                                                                                              |    Yes    |    -     | Math: `custom_so_ordered_qty - custom_previously_disp_qty - qty`. Must be $\ge 0$.        |
| `custom_scanned_count`            | Barcodes Scanned Count      | `Int`     | -                                                                                                                              |    Yes    |    -     | Number of unique serial numbers bound in SABB. Must equal `qty` if serialized.            |
| `custom_serial_validation_passed` | Serial Validation Passed    | `Check`   | -                                                                                                                              |    Yes    |    -     | Flag: 1 if all bound serials are verified in source warehouse stock with zero duplicates. |
...
In Frappe Framework v15+, serial tracking was re-architected from flat text strings into relational **Serial and Batch Bundles** (`tabSerial and Batch Bundle` and child table `tabSerial and Batch Entry`).
...
2. **Cardinality Match:** $\text{len}(entries) == \text{round}(item.qty)$. Partial serial allocations are hard-blocked.
3. **No Duplicate Serials:** No serial number can appear twice across the same Delivery Note or in any other pending unsubmitted Delivery Note.
4. **Item Code Conformance:** Every serial number's parent item code in `tabSerial No` must strictly match `item.item_code`.

---

### 3.4 Custom Child DocType: `tabSolar Dispatch Checklist Item`

Pre-dispatch quality and physical staging checklist attached to `tabDelivery Note`.
...
| :----------------------- | :--------------------- | :----------- | :------------------------------------------------------------------------ | :-------: | :---------------------------------------------------------------------------- |
| `checkpoint_code`        | Checkpoint Code        | `Data`       | -                                                                         |    Yes    | Unique checkpoint identifier (e.g. `CHK-MOD-01`, `CHK-INV-01`, `CHK-CBL-01`). |
| `checkpoint_description` | Inspection Requirement | `Small Text` | -                                                                         |    Yes    | Detailed description of physical inspection standard.                         |
...
| `photo_evidence`         | Photographic Evidence  | `Attach`     | -                                                                         |    No     | Photo of seal, carton, or structural finish.                                  |
| `verified_by`            | Inspector User         | `Link`       | `User`                                                                    |    No     | User ID of inspector marking row passed.                                      |
| `verified_on`            | Inspection Timestamp   | `Datetime`   | -                                                                         |    No     | Timestamp of checklist verification.                                          |
| `remarks`                | Inspector Notes        | `Data`       | -                                                                         |    No     | Notes regarding packaging condition or defect observed.                       |

#### Mandatory Standard Checkpoints Seeded Automatically:

1. `CHK-MOD-01`: **Module Physical Integrity:** Inspect all PV module cartons. Zero cracked glass, bent aluminum frames, or damaged junction boxes.
2. `CHK-INV-01`: **Inverter Factory Seal & Kit:** Verify OEM tamper-evident seals intact, wall-mounting bracket, Wi-Fi/4G datalogger, and DC connector kit enclosed.
3. `CHK-MMS-01`: **Structure Hot-Dip Galvanization:** Inspect MMS channels and purlins for zinc coating completeness; verify hardware bolt counts match BOM.
4. `CHK-CBL-01`: **Solar Cable Drum Seals:** Verify DC (4/6 sq.mm) and AC cable drum factory seal, conductor cross-section, and ISI/IEC test marking.
5. `CHK-TRN-01`: **Transit Insurance & Weatherproofing:** Verify cargo truck is covered with waterproof tarpaulin and transit insurance policy covers consignment value.

---

### 3.5 Standalone Single DocType: `tabSolar Dispatch Settings`

Global operational parameters governed strictly by `Admin` and `System Manager`.

| Fieldname                          | Label                          | Fieldtype            |          Default          | Description                                                                 |
| :--------------------------------- | :----------------------------- | :------------------- | :-----------------------: | :-------------------------------------------------------------------------- |
| `default_dispatch_sla_hours`       | Target Dispatch SLA Hours      | `Int`                |           `48`            | Target turnaround time from Stage 06 SO submit to Stage 07 DN submit.       |
| `sla_warning_threshold_hours`      | SLA Warning Alert Threshold    | `Int`                |           `12`            | Hours before SLA deadline to broadcast warning notifications.               |
| `eway_bill_mandatory_threshold`    | E-Way Bill Mandatory Value     | `Currency`           |         `50000.0`         | Statutory minimum consignment value requiring mandatory E-Way Bill.         |
| `enforce_strict_serial_scan`       | Enforce Strict Serial Scan     | `Check`              |            `1`            | If 1, dispatches containing modules/inverters cannot submit without SABB.   |
| `allow_partial_dispatch`           | Support Phased Dispatches      | `Check`              |            `1`            | Enables multi-stage consignments (Civil, Panels, Inverters).                |
...
| `mandate_pre_dispatch_checklist`   | Mandate Inspection Checklist   | `Check`              |            `1`            | If 1, all checklist items must be Passed before DN submission.              |
| `in_transit_virtual_warehouse`     | Virtual Goods-in-Transit Store | `Link` (`Warehouse`) | `Goods In Transit - SEPC` | Intermediate warehouse ledger for stock moving from store to site.          |
| `notify_customer_on_dispatch`      | WhatsApp/SMS Alert to Customer | `Check`              |            `1`            | Dispatches tracking alert to customer mobile upon DN submission.            |
| `notify_site_engineer_on_dispatch` | Alert Receiving Site Team      | `Check`              |            `1`            | Alerts Project Engineer and Site Supervisor with vehicle & driver contact.  |
...
The following 6 verification gates execute sequentially inside `DeliveryNoteSolarController.validate()` and `before_submit()`:
...
For every item line $i$ across any Delivery Note linked to Sales Order $SO$, the system computes:
...
Where $\mathcal{T}_{\text{tol}}$ is the Admin-governed tolerance percentage (`max_over_dispatch_threshold_pct`, default $0.0\%$).
...
```
Stage 06 SO Submitted ──▶ Store Task Created (Timestamp: T_0)
Target SLA: T_0 + 48 Hours
```
...
Following [`architect_docs/05_DESIGN_PATTERNS_DOMAIN_SERVICES_SOLID.md`](../architect_docs/05_DESIGN_PATTERNS_DOMAIN_SERVICES_SOLID.md), controllers remain thin orchestrators. All domain math, validation rules, barcode ingestion, and POD workflows are encapsulated in dedicated service classes:
...
class SolarPODService:
    ...
    def _cascade_pod_completion(dn):
        ...
        if dn.custom_project_ref:
            # Check if all SO items are delivered
...
All endpoints are strictly whitelisted with HTTP POST restrictions, type hints, and permission checks.
...
In strict alignment with the **Universal Frontend Landing Architecture** ([`step_plans/README.md#2-universal-frontend-landing--routing-contract`](./README.md#2-universal-frontend-landing--routing-contract)), all primary store dispatch workflows run on `/solar` (Vue 3 + Frappe UI SPA), supplemented by permission-gated Frappe Desk form views.
...
Designed for warehouse operators using handheld Zebra/Honeywell 2D scanners or camera-enabled mobile tablets:
...
Optimized for mobile smartphones used on-site by `Project Engineer` / `Site Supervisor`:
...
For power users accessing desk deep links (`/app/delivery-note/DN-YYYY-XXXXX`):

1. **Dual Progress Bar Header Banner:** Displays active position on the Project Stepper (`[05G. Advance] -> [06. SO Baseline] -> [★ 07. DISPATCH ACTIVE ★] -> [08. Installation] -> ...`).
...
3. **Desk Form Custom Buttons:**
   - `[Open Scanning Workbench]`: Deep-links directly to `/solar/store/scan/<id>`.
   - `[Validate E-Way Bill]`: Verifies GST portal status via API.
   - `[Send Dispatch Alert (WhatsApp)]`: Triggers instant customer/site WhatsApp notification.
...
Stage 07 orchestrates transactions across ERPNext Core, Frappe CRM, Frappe HRMS, and external statutory portals:
...
- **Stock Ledger Impact:** Submitting `Delivery Note` (`docstatus = 1`) creates Stock Ledger Entries (SLE) crediting the source warehouse (`Central Store Warehouse - SEPC`) and debiting inventory valuation.
- **In-Transit Virtual Accounting:** If `in_transit_virtual_warehouse` is enabled in `Solar Dispatch Settings`, stock moves to a virtual in-transit holding warehouse until the digital POD is confirmed at site.
- **Sales Order Synchronization:** Updates `tabSales Order.per_delivered` in real time. When all items reach 100%, the Sales Order status transitions to `Completed`.

### 7.2 Frappe CRM Customer Notification

Upon Delivery Note submission, an asynchronous background job (`solar_module.tasks.send_dispatch_alert`) sends an automated WhatsApp message to the customer:
...
- Generates compliant E-Way Bill JSON payloads matching Indian National Informatics Centre (NIC) GST standards for Part A (Doc No, Value, HSN Codes) and Part B (Vehicle Registration Plate, Transporter ID).

---

## 8. Automated Testing & QA Criteria

In accordance with [`architect_docs/07_AUTOMATED_TESTING_QA_CI_CD.md`](../architect_docs/07_AUTOMATED_TESTING_QA_CI_CD.md), the test suite strictly enforces the **Zero-Commit Rule** (`frappe.db.rollback()`), guaranteeing test isolation and zero database pollution.
...
| :------------------------------------------------------------------- | :------------------------------------------------------------------------- | :--------------------------------------------------------------------------------------------------------- |
| `Originating Sales Order baseline is not frozen`                     | Commercial sales team has not finalized and frozen Stage 06.               | Contact Sales / CRM Manager to submit and lock the baseline in Stage 06.                                   |
...
| :----------------------------------- | :-------: | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Canonical 9 Sections Complete**    | ✅ Passed | Every section from Blueprint [`architect_docs/02_STEP_PLANNING_SPECIFICATION_BLUEPRINT.md`](../architect_docs/02_STEP_PLANNING_SPECIFICATION_BLUEPRINT.md) fully detailed. |
| **Zero "User" Suffix Rule Enforced** | ✅ Passed | All personas use approved titles (`Store Manager`, `Store Assistant`, `Project Engineer`).                                                                                 |
